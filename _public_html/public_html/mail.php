...
if (version_compare(PHP_VERSION, '5.1.0', '>=')) {//PHP5.1.0以上の場合のみタイムゾーンを定義
	date_default_timezone_set('Pacific/Honolulu');//タイムゾーンの設定（日本以外の場合には適宜設定ください）
}
/*-------------------------------------------------------------------------------------------------------------------
* ★以下設定時の注意点　
...
$Referer_check_domain = "fukupa.com";
...
$thanksPage = "http://fukupa.com/thanks.html";
...
$re_subject = "Thank you for your message";
...
Thank you for your message / inquiry.
...
Hawaii [Head Office]
99-950 Iwaena St. #B, Aiea, HI 96701
TEL:(808) 486-6616
FAX:(808) 488-4808
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Email && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
				$errm .= "<p class=\"error_messe\">【".$key."】Invalid email address. Please fill in valid
email address.</p>\n";
				$empty_flag = 1;
			}
		}
...
<?php if($empty_flag == 1){ ?>
<div align="center">
<h4>Please fill out required field(s).</h4>
<?php echo $errm; ?><br /><br /><input type="button" value=" Go back to previous page " onClick="history.back()">
</div>
<?php }else{ ?>
<h3 align="center">Confirmation</h3>
<!--p align="center">Confirm your submission</p-->
...
<input type="button" value="Go back to previous page" onClick="history.back()"></p>
</form>
<?php } ?>
</div>
<div style="width:300px; margin:50px auto 0 auto;"><img src="images/s_logo.png"></div>
<!-- /formWrap -->
<!-- ▲ *********** 送信内容確認部　※編集は自己責任で ************ ▲-->
...
<div align="center">
<?php if($empty_flag == 1){ ?>
<h4>Please fill out required field(s).
</h4>
<div style="color:red"><?php echo $errm; ?></div>
<br /><br /><input type="button" value=" Go back to previous page " onClick="history.back()">
</div>
</body>
</html>
<?php }else{ ?>
Thank you. Submission completed<br /><br />
<a href="<?php echo $site_top ;?>">Go back to Top page&raquo;</a>
</div>
<?php copyright(); ?>
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4>Please fill out required field(s).</h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" value=" Go back to previous page " onClick="history.back()"></div>
<?php 
...
function mailToAdmin($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	...
	$adminBody.="\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n";
	$adminBody.="Message sent：".date( "Y/m/d (D) H:i:s", time() )."\n";
	$adminBody.="Sender’s IP address：".@$_SERVER["REMOTE_ADDR"]."\n";
	$adminBody.="Sender’s Host：".getHostByAddr(getenv('REMOTE_ADDR'))."\n";
	if($confirmDsp != 1){
		$adminBody.="Sent pages URL：".@$_SERVER['HTTP_REFERER']."\n";
	}else{
		$adminBody.="Sent pages URL：".@$arr['httpReferer']."\n";
	}
...
function mailToUser($arr,$dsp_name,$remail_text,$mailFooterDsp,$mailSignature,$encode){
	...
	$userBody.="\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n\n";
	$userBody.="Message sent：".date( "Y/m/d (D) H:i:s", time() )."\n";
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\">Please fill in【".h($key)."】</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
				elseif($val == ''){
					$res['errm'] .= "<p class=\"error_messe\">Please fill in your【".h($key)."】</p>\n";
					$res['empty_flag'] = 1;
				}
		...
		if($existsFalg != 1){
				$res['errm'] .= "<p class=\"error_messe\">Please fill in【".$requireVal."】</p>\n";
				$res['empty_flag'] = 1;
		}
	}
...
function refererCheck($Referer_check,$Referer_check_domain){
	if($Referer_check == 1 && !empty($Referer_check_domain)){
		if(strpos($_SERVER['HTTP_REFERER'],$Referer_check_domain) === false){
			return exit('<p align="center">Referrer check error</p>');
		}
	}
}
function copyright(){
	echo '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="http://www.fukupa.com/">- Fukuoka Package USA, INC. -</a>';
}
//----------------------------------------------------------------------
//  関数定義(END)
//----------------------------------------------------------------------
